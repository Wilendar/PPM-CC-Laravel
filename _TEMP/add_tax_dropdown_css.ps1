...
# CSS block to add (before "RESPONSIVE ADJUSTMENTS" section)
$cssBlock = @"
/* ========================================
...
/* Default option - GREEN (zgodność z PPM default) */
.tax-option-default {
...
# Find the insertion point (before RESPONSIVE ADJUSTMENTS)
$marker = '/* ========================================'
$responsiveSection = '   RESPONSIVE ADJUSTMENTS'

# Find position
$insertPosition = $content.IndexOf($marker + "`r`n" + $responsiveSection)

if ($insertPosition -gt 0) {
    # Insert CSS block before RESPONSIVE ADJUSTMENTS
    $content = $content.Insert($insertPosition, $cssBlock)
    Write-Host "CSS block inserted successfully" -ForegroundColor Green
} else {
    Write-Host "ERROR: Could not find RESPONSIVE ADJUSTMENTS section" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "SUCCESS: Tax dropdown CSS styles added!" -ForegroundColor Green
Write-Host ""
