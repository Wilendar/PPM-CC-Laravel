...
class ProductTransformer
{
    ...
    public function transformForPrestaShop(Product $product, BasePrestaShopClient $client): array
    {
        ...
        $prestashopProduct = [
            'product' => [
                ...
                // REQUIRED FIELDS for admin panel visibility (BUGFIX 2025-11-06)
                // Discovered through field-by-field comparison: product 9755 (visible) vs 9762 (invisible)
                // Without these fields, product exists in DB but is INVISIBLE in admin panel
                'minimal_quantity' => 1,
                ...
                'associations' => [
                    'categories' => $categoryAssociations = $this->buildCategoryAssociations($product, $shop),
                ],

                // Default category (required by PrestaShop - first category or fallback to Home)
                'id_category_default' => $categoryAssociations[0]['id'] ?? 2,
                ...
                // Manufacturer (BUGFIX 2025-11-05: use id_manufacturer, not manufacturer_name - readonly!)
     ...
     *
     * BUGFIX 2025-11-06: Price must be > 0 for admin panel visibility
     * Zero-price products are filtered out by PrestaShop admin list queries
     *
     * @param Product $product Product instance
     * @param PrestaShopShop $shop Shop instance
     * @return float Net price (minimum 0.01)
     */
    private function calculatePrice(Product $product, PrestaShopShop $shop): float
    {
        ...
        if (!$price) {
            Log::warning('No price found for default group - using minimum price', [
                'product_id' => $product->id,
                'shop_id' => $shop->id,
                'price_group_id' => $defaultPriceGroup->id,
            ]);
            return 0.01; // Minimum price required for admin visibility
        }

        // Return net price (PrestaShop calculates gross based on tax rules)
        // Ensure minimum price of 0.01 (zero-price products invisible in admin)
        return max(0.01, (float) $price->price_net);
    }
     ...
     *
     * BUGFIX 2025-11-05: manufacturer_name is READONLY in PrestaShop API
     * Must use id_manufacturer instead
     *
     * BUGFIX 2025-11-06: id_manufacturer MUST NOT be NULL or 0
     * Products with NULL/0 manufacturer are INVISIBLE in PrestaShop admin panel
     *
     * Searches for existing manufacturer or creates new one if not found
     * Falls back to default manufacturer ID if all attempts fail
     *
     * @param string|null $manufacturerName Manufacturer name from PPM
     * @param BasePrestaShopClient $client PrestaShop API client
     * @param PrestaShopShop $shop Shop instance
     * @return int Manufacturer ID (NEVER null)
     */
    private function getManufacturerId(
        ...
        // BUGFIX 2025-11-06: If no manufacturer name, use default
        // NULL manufacturer = invisible product in admin panel
        if (!$manufacturerName) {
            Log::warning('No manufacturer name - using default', [
                'shop_id' => $shop->id,
            ]);
            return config('prestashop.default_manufacturer_id', 1);
        }
        ...
        // Manufacturer not found - create new one
        try {
            $xmlData = [
                'manufacturer' => [
        ...
        // BUGFIX 2025-11-06: If all fails, return default manufacturer ID
        // NEVER return null - products with NULL manufacturer are INVISIBLE in admin panel
        Log::warning('All manufacturer operations failed - using default', [
            'manufacturer_name' => $manufacturerName,
            'shop_id' => $shop->id,
        ]);
