...
class BaselinkerService implements ERPSyncServiceInterface
{
    ...
    */
     ...
     *
     * Used during Baselinker import to set the default PPM status
     * on products that don't have one yet. Uses saveQuietly()
     * to avoid triggering observers during import.
     */
    protected function assignDefaultProductStatus(Product $product): void
    {
        if (!$product->product_status_id) {
            $defaultStatus = ProductStatus::getDefault();
            if ($defaultStatus) {
                $product->product_status_id = $defaultStatus->id;
                $product->saveQuietly();
            }
        }
    }
    ...
    public function syncProductFromERP(ERPConnection $connection, string $erpProductId): array
    {
        ...
        try {
            ...
            if ($product) {
                ...
                // Assign default product status for newly imported products
                $this->assignDefaultProductStatus($product);
