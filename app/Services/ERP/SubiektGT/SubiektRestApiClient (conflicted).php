...
class SubiektRestApiClient
{
    ...
    /**
     * Get latest change info from tw_ZmianaTw table.
     * NOTE: twz_NrZmiany is per-product (NOT global sequence).
     * Use latest_change_time (twz_CzasModyf) as the delta marker.
     *
     * @return array{latest_change_number: int, latest_change_time: string, total_changes: int}
     * @throws SubiektApiException
     */
    public function getLatestChangeInfo(): array
    {
        $response = $this->request('GET', '/api/changes/latest');
    ...
    /**
     * Get SKUs of products changed since a given timestamp.
     * Uses tw_ZmianaTw.twz_CzasModyf (datetime) for delta detection.
     *
     * @param string $sinceTime ISO datetime string (e.g. '2026-03-11T12:00:00')
     * @param int $limit Max results (default 500)
     * @return array Array of changed products [{sku, change_number, change_time, product_id}, ...]
     * @throws SubiektApiException
     */
    public function getChangedSkusSince(string $sinceTime, int $limit = 500): array
    {
        $response = $this->request('GET', '/api/changes/since', [
            'since' => $sinceTime,
            'limit' => $limit,
        ]);
        return $response['data'] ?? [];
    ...
    /**
     * Get MD5 checksums of stock data for given SKUs.
     * Used for stock delta detection (stock has no timestamp).
     *
     * @param array $skus Array of SKU strings
     * @param int $warehouseId Warehouse ID (default 1)
     * @return array Associative array [SKU => checksum_hash]
     * @throws SubiektApiException
     */
    public function getStockChecksums(array $skus, int $warehouseId = 1): array
    {
        if (empty($skus)) {
            return [];
        }

        // Ensure all SKUs are strings (numeric SKUs like 100010670 cause JSON deserialization errors)
        $response = $this->request('POST', '/api/checksum/stock', [
            'skus' => array_map('strval', array_values($skus)),
            'warehouseId' => $warehouseId,
        ]);
