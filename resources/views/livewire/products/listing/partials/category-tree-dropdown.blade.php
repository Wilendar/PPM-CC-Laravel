{{-- Category Tree Dropdown - hierarchical category filter --}}
@php
    $treeCategories = \App\Models\Category::select(['id', 'name', 'parent_id', 'level'])
...
<div x-data="categoryTreeDropdown({{ json_encode($categoryTree) }}, '{{ $this->categoryFilter }}')"
...
@script
<script>
Alpine.data('categoryTreeDropdown', (tree, initialFilter) => ({
    open: false,
    searchTerm: '',
    expandedNodes: [],
    ...
    selectCategory(id, label) {
        ...
        this.$wire.set('categoryFilter', id ? String(id) : '');
        this.open = false;
        this.searchTerm = '';
    }
}));
