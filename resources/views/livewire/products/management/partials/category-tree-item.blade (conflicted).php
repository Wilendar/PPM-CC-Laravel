    ...
    @if($hasAnyChildren)
        <div x-show="!collapsed"
            ...
            @if($hasUnprunedChildren)
                ...
                {{-- excludeIds: server-rendered children already visible (prevents duplicates) --}}
                @php
                    $alreadyRenderedIds = $hasChildren ? $category->children->pluck('id')->toArray() : [];
                @endphp
                <div x-data="categoryTreeLazy({
                    parentId: {{ $category->id }},
                    context: '{{ $context }}',
                    level: {{ $level + 1 }},
                    excludeIds: @js($alreadyRenderedIds)
                })" x-init="loadChildren()"
