            ...
            <div class="category-form-left-column">
                ...
                <div class="{{ $activeTab === 'basic' ? '' : 'hidden' }}">
                        ...
                        <div class="mb-4 p-4 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800 shadow-sm">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    @if($syncDisplay['prestashop_id'])
                                        <a href="{{ $currentShop['shop_url'] }}/admin-dev/index.php?controller=AdminProducts&id_product={{ $syncDisplay['prestashop_id'] }}"
                                           target="_blank"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="{{ $this->getCategoryClasses() }} max-h-64 overflow-y-auto" wire:key="categories-ctx-{{ $activeShopId ?? 'default' }}">
                                    @foreach($availableCategories as $category)
                                        <div class="flex items-center space-x-2 py-1" wire:key="category-row-{{ $activeShopId ?? 'default' }}-{{ $category->id }}">
                                            ...
                                            <label for="category_{{ $activeShopId ?? 'default' }}_{{ $category->id }}" class="flex-1 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        {{-- PRESTASHOP CATEGORIES SECTION (ETAP_07 FAZA 2B.2) --}}
                        @if($activeShopId !== null)
                            <div class="md:col-span-2">
                                <div class="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
                                    <div class="flex items-center justify-between mb-4">
                                        <h4 class="text-base font-semibold text-gray-900 dark:text-white flex items-center">
                                            <svg class="w-5 h-5 mr-2 text-mpp-orange" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3.93a2 2 0 01-1.66-.9l-.82-1.2A2 2 0 0011.93 4H5a2 2 0 00-2 2v1m0 0h18" />
                                            </svg>
                                    ...
                                    {{-- Loading state --}}
                                    <div wire:loading wire:target="loadPrestaShopCategories,refreshPrestaShopCategories">
                                        <div class="flex items-center justify-center py-8">
                                            <div class="flex items-center space-x-3 text-gray-400">
                                                ...
                                                <span class="text-sm">Ładowanie kategorii z PrestaShop...</span>
                                            </div>
                                        </div>
                                    </div>

                                    {{-- Category Tree (after load) --}}
                                    <div wire:loading.remove wire:target="loadPrestaShopCategories,refreshPrestaShopCategories">
                                        @if(isset($prestashopCategories[$activeShopId]))
                                            <div class="enterprise-card p-4 max-h-96 overflow-y-auto bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
                                                @foreach($prestashopCategories[$activeShopId] as $category)
                                            ...
                                            @if(isset($shopData[$activeShopId]['prestashop_categories']) && count($shopData[$activeShopId]['prestashop_categories']) > 0)
                                                <div class="mt-4">
                                                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-2 font-medium">
                                                        Wybrane kategorie PrestaShop ({{ count($shopData[$activeShopId]['prestashop_categories']) }}):
                                                    </p>
                                                    <div class="flex flex-wrap gap-2">
                                                        @foreach($shopData[$activeShopId]['prestashop_categories'] as $catId)
                                                            @php
                                                                $categoryName = $this->getCategoryName($activeShopId, $catId);
                                                            @endphp
                                                            <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                                                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        ...
                                        @else
                                            <div class="text-center py-8">
                                                ...
                                                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400 italic">
                                                    Kliknij "Odśwież kategorie" aby pobrać strukturę kategorii z PrestaShop
                                                </p>
                                            </div>
                                        @endif
                                    </div>
