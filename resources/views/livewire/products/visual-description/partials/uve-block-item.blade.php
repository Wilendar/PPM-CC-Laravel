        ...
        @else
            ...
            @if(!empty($block['compiled_html']))
                <div class="uve-block-preview">
                    {{-- @safe: compiled_html is generated by BlockRenderer from internal block document structure.
                         Blocks imported from PrestaShop are parsed and re-rendered through the block system,
                         not injected raw. If direct PrestaShop HTML blocks are ever added, sanitize here. --}}
                    {!! $block['compiled_html'] !!}
                </div>
            @else
                <div class="uve-block-placeholder">
